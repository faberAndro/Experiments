...
#  KAGGLE SCORE IS INCORRECT AND NEED AMENDMENTS (AND LOOPING TROUGH ALL THE TRAIN SET)
...
#  implement this model in Google COLAB
...
def evaluate_kaggle_score(w, y_inv):
    ...
    probabilities = w * y_inv * (y_inv > 0)     # this formula the N.2 y_inv should come from different sources: to fix
    probabilities_sum = probabilities.sum()
    probabilities_var = np.var(probabilities)
    t = probabilities_sum/probabilities_var * np.sqrt(250/1)
    u = min(max(t, 0), 6)*probabilities_sum     # include properly the dates
    return u
